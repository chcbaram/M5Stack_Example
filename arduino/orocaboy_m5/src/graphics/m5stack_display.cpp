...
const gpio_num_t LCD_PIN_NUM_RESET = GPIO_NUM_33;
...
static const ili_init_cmd_t ili_init_cmds[] = {
    ...
    //{0x36, {(MADCTL_MV | MADCTL_MX | TFT_RGB_BGR)}, 1},    // Memory Access Control
    {0x36, {(TFT_RGB_BGR)}, 1},    // Memory Access Control
    //{0x36, {0xA8}, 1},    // Memory Access Control
    {0x3A, {0x55}, 1},
    {0xB1, {0x00, 0x13}, 2},  // Frame Rate Control (1B=70, 1F=61, 10=119), 1B
...
static void ili_init()
{
  ...
  gpio_set_direction(LCD_PIN_NUM_RESET, GPIO_MODE_OUTPUT);
  

  gpio_set_level(LCD_PIN_NUM_RESET, 1);
  vTaskDelay(10 / portTICK_RATE_MS);
  gpio_set_level(LCD_PIN_NUM_RESET, 0);
  vTaskDelay(10 / portTICK_RATE_MS);
  gpio_set_level(LCD_PIN_NUM_RESET, 1);
...
void send_continue_line(uint16_t *line, int width, int lineCount)
{
  ...
  trans[6].tx_data[0] = 0x2C;           //memory write continue
