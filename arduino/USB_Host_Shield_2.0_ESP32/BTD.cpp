...
static void controller_rcv_pkt_ready(void)
{
  ...
  //printf(".\n");
  //Serial.println("controller rcv pkt ready");
}
...
void BTD::HCI_event_task(uint8_t *p_data, uint16_t param_length) {
        ...
        if(!rcode || rcode == hrNAK) { // Check for errors
                switch(hcibuf[0]) { // Switch on event type
                        ...
                        printf("EV_REMOTE_NAME_COMPLETE\r\n");
                                ...
                                {
#ifdef DEBUG_USB_HOST
                                        printf("remote name filed\r\n ");                                        
#endif                                        
                                }                                
                                break;

                        case EV_INCOMING_CONNECT:
                        printf("EV_INCOMING_CONNECT\r\n");
                                printf("disc_bdaddr ");
                                for(uint8_t i = 0; i < 6; i++)
                                {
                                        disc_bdaddr[i] = hcibuf[i + 2];
                                        printf("0x%X ", disc_bdaddr[i]);                                        
                                }
                                printf("\r\n");
        ...
        switch(hci_state) {
...
#ifdef DEBUG_USB_HOST
                                Notify(PSTR("\r\nHCI_SET_NAME_STATE: "), 0x80);
#endif                                        
                                } else
                                {
                                        hci_state = HCI_CHECK_DEVICE_SERVICE;
#ifdef DEBUG_USB_HOST
                                Notify(PSTR("\r\HCI_CHECK_DEVICE_SERVICE: "), 0x80);
#endif                                        
                ...
                case HCI_CONNECT_IN_STATE:
                        if(hci_check_flag(HCI_FLAG_INCOMING_REQUEST)) {
                                ...
                                //hci_remote_name();
                                //hci_state = HCI_REMOTE_NAME_STATE;
                                // 일단 REMOTE NAME은 무시하자, 나중에 디버깅. 이걸 활성화 하면 재연결이 안됨.
...
void BTD::HCI_Command(uint8_t* data, uint16_t nbytes) {
  ...
  {    
    ...
    {
      bt_tx_buf[0] = H4_TYPE_COMMAND;
      for (int i=0; i<nbytes; i++)
      {
        bt_tx_buf[1+i] = data[i];
      }
...
void BTD::L2CAP_Command(uint16_t handle, uint8_t* data, uint8_t nbytes, uint8_t channelLow, uint8_t channelHigh) {
        ...
        uint8_t rcode = 0; // = pUsb->outTransfer(bAddress, epInfo[ BTD_DATAOUT_PIPE ].epAddr, (8 + nbytes), buf);
        ...
        {    
          if (esp_vhci_host_check_send_available())
          {
            bt_tx_buf[0] = H4_TYPE_ACL;
            for (int i=0; i<nbytes+8; i++)
            {
              bt_tx_buf[1+i] = buf[i];
            }
            esp_vhci_host_send_packet(bt_tx_buf, nbytes+8+1);
            break;
          }
        }        
