...
const uint8_t XBOXOLD_BUTTONS[] PROGMEM = {
        ...
        // A, B, X, Y, BLACK_, WHITE_, L1, and R1 are analog buttons
        4, // BLACK_
        5, // WHTIE
        6, // L1
        7, // R1
...
void XBOXOLD::readReport() {
        ...
        for(uint8_t i = 0; i < sizeof (buttonValues); i++)
                buttonValues[i] = readBuf[i + 4]; // A, B, X, Y, BLACK_, WHITE_, L1, and R1
        ...
        if(ButtonState != OldButtonState || memcmp(buttonValues, oldButtonValues, sizeof (buttonValues)) != 0) {
                ...
                for(uint8_t i = 0; i < sizeof (buttonValues); i++) {
                        if(oldButtonValues[i] == 0 && buttonValues[i] != 0)
                                buttonClicked[i] = true; // Update A, B, X, Y, BLACK_, WHITE_, L1, and R1 click state
...
uint8_t XBOXOLD::getButtonPress(ButtonEnum b) {
        ...
        if(b == A || b == B || b == X || b == Y || b == BLACK_ || b == WHITE_ || b == L1 || b == R1) // A, B, X, Y, BLACK_, WHITE_, L1, and R1 are analog buttons
